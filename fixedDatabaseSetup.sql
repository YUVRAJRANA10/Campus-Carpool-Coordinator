-- Fixed database setup - no column reference errors
...
-- 5. Create reviews table for post-trip ratings (simplified)
...
-- 7. Create simple RLS policies for reviews (without column references)
...
-- 16. Check live_rides table structure
SELECT 
    column_name, 
    data_type, 
    is_nullable 
FROM information_schema.columns 
WHERE table_name = 'live_rides' 
    AND table_schema = 'public'
ORDER BY ordinal_position;
